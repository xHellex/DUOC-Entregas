    ...
    public CuentaInteres process(InteresInput input) {
        ...
        if (saldo == null || saldo <= 0) {
            throw new InvalidDataException(
                    "Saldo invalido en cuenta id=" + input.getCuentaId() + ": '" + input.getSaldo() + "'");
        }
        ...
        if (edad == null || edad < EDAD_MIN || edad > EDAD_MAX) {
            throw new InvalidDataException(
                    "Edad fuera de rango en cuenta id=" + input.getCuentaId() + ": '" + input.getEdad() + "'");
        }
        ...
        if (tasa == null) {
            throw new InvalidDataException(
                    "Tipo desconocido en cuenta id=" + input.getCuentaId() + ": '" + input.getTipo() + "'");
        }
